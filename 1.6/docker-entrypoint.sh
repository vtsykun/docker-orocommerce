...
docker_install_application() {
  ...
  local dbDriver=$(cat app/config/parameters.yml | awk '/database_driver:/{ print $2 }')

  if [[ "$dbDriver" == "pdo_pgsql" ]]; then
    sudo -u www-data php app/console doctrine:query:sql 'CREATE EXTENSION IF NOT EXISTS "uuid-ossp"' -vvv --env=prod || true
  fi

  sudo --preserve-env=LD_PRELOAD -u www-data php app/console oro:install --env=prod --user-name="${ADMIN_USER}" --user-email="${ADMIN_EMAIL}" \
...
chown www-data:www-data /var/www/orocommerce/app -R
chown www-data:www-data /var/www/orocommerce/web -R
...
if [[ "$ORO_INSTALLED" == "false" ]]; then
  ...
  sudo -u www-data php app/console --env=prod > /dev/null || true
  sudo -u www-data php app/console about --env=prod || true

  docker_install_application
else
  echo 'Clear cache'
  sed -i "s/installed"\:".*/installed"\:" true/g" app/config/parameters.yml
  sudo -u www-data php app/console --env=prod > /dev/null || true
  sudo -u www-data php app/console about --env=prod || true
  ...
  if [[ ! -f '/.oro-installed' ]]; then
    ...
    sudo --preserve-env=LD_PRELOAD -u www-data php app/console oro:platform:update --env=prod \
